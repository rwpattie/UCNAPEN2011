...
C    PMFILE .... array of MAXMAT 20-character strings. The first NMAT
...
C  ****  Elastic scattering of electrons and positrons.
     ...
     2  EJTL(NEGP),FJL(NEGP),A(NEGP),B(NEGP),C(NEGP),D(NEGP)
...
C  ****  Photon simulation tables.
     ...
     1  SGPH(MAXMAT,NEGP),SGPP(MAXMAT,NEGP),SGAUX(MAXMAT,NEGP)
      ...
      COMMON/CGPP01/TRIP(MAXMAT,NEGP),PTRSH(MAXMAT,NOCO)
C  ****  Auxiliary arrays.
      DIMENSION EIT(NEGP),EITL(NEGP),FL(NEGP),F1(NEGP),F2(NEGP),
     1  F3(NEGP),F4(NEGP),RADY(NEGP),RADN(NEGP)
...
C
      DO I=1,NEGP
        ...
        ENDIF
        RSTPE(M,I)=STPR*FACT
        TSTPE(M,I)=LOG(CSTPE(M,I)+RSTPE(M,I))
        TSTRE(M,I)=LOG(TSTRE(M,I)+(XS2+XH2)*VMOL(M)*FACT)
...
C
      DO I=1,NEGP
        ...
        ENDIF
        RSTPP(M,I)=STPR*FACT
        TSTPP(M,I)=LOG(CSTPP(M,I)+RSTPP(M,I))
        TSTRP(M,I)=LOG(TSTRP(M,I)+(XS2+XH2)*VMOL(M)*FACT)
...
C
      ...
      DO I=1,NEGP
        TRL1E(M,I)=-LOG(XE1(I)*VMOL(M))
        TRL2E(M,I)=-LOG(XE2(I)*VMOL(M))
        TRL1P(M,I)=-LOG(XP1(I)*VMOL(M))
        TRL2P(M,I)=-LOG(XP2(I)*VMOL(M))
      ENDDO
...
C  ****  Pairs.
      ...
      DO 1 I=1,NDATA
        ...
        NP=NP+1
        F1(NP)=EITL(I)
        FACT=(EIT(I)/(EIT(I)-1.022D6))**3
        FL(NP)=LOG(FACT*MAX(F3(I),1.0D-35))
    1 CONTINUE
      ...
      DO I=1,NEGP
        IF(ET(I).LT.1.023D6) THEN
          SGPP(M,I)=-80.6D0
        ELSE
          FACT=(ET(I)/(ET(I)-1.022D6))**3
          EC=DLEMP(I)
          CALL FINDI(F1,EC,NP,J)
          SGPP(M,I)=A(J)+EC*(B(J)+EC*(C(J)+EC*D(J)))+VMOLL-LOG(FACT)
        ENDIF
      ENDDO
C  ****  Triplets.
      NP=0
      DO 2 I=1,NDATA
        ...
        NP=NP+1
        FACT=(EIT(I)/(EIT(I)-2.044D6))**3
        F1(NP)=EITL(I)
        FL(NP)=LOG(FACT*MAX(F4(I),1.0D-35))
    2 CONTINUE
      CALL SPLINE(F1,FL,A,B,C,D,0.0D0,0.0D0,NP)
      DO I=1,NEGP
        IF(ET(I).LT.2.045D6) THEN
          TRIP(M,I)=-80.6D0
        ELSE
          FACT=(ET(I)/(ET(I)-2.044D6))**3
          EC=DLEMP(I)
          CALL FINDI(F1,EC,NP,J)
          TRIPL=EXP(A(J)+EC*(B(J)+EC*(C(J)+EC*D(J)))+VMOLL-LOG(FACT))
          PAIR=EXP(SGPP(M,I))
          SGPP(M,I)=LOG(PAIR+TRIPL)
          TRIP(M,I)=TRIPL/(PAIR+TRIPL)
        ENDIF
      ENDDO
C
      IF(NOSCCO(M).GT.1) THEN
        PTRSH(M,1)=FCO(M,1)
        DO I=2,NOSCCO(M)
          PTRSH(M,I)=PTRSH(M,I-1)+FCO(M,I)
        ENDDO
        DO I=1,NOSCCO(M)
          PTRSH(M,I)=PTRSH(M,I)/PTRSH(M,NOSCCO(M))
        ENDDO
      ELSE
        PTRSH(M,1)=1.0D0
      ENDIF
...
C  Only the K, L, M and N shells with ionisation energies greater than
C  that of the N7 shell of the heaviest element in the material are
C  considered as inner shells. As a result, the inner/outer character
C  of an atomic shell depends on the composition of the material.
C
      IZMAX=0
      DO I=1,NELEM(M)
        IZMAX=MAX(IZMAX,IZ(M,I))
      ENDDO
      WISCUT=MAX(50.0D0,EB(IZMAX,16)-1.0D0)
C
      NOST=NOS
      DO I=1,NOST
        FFT(I)=FF(I)
  ...
  910 CONTINUE
      ...
      IF(NOST.GT.IFCB+2) THEN
        DO 911 I=IFCB+1,NOST-1
          IF(KST(I).LT.17.AND.UIT(I).GT.WISCUT) GO TO 911
...
C
  920 CONTINUE
      ...
      IF(NOSTC.GT.2) THEN
        DO 921 I=1,NOSTC-1
          IF(KSC(I).LT.17.AND.UIC(I).GT.WISCUT) GO TO 921
...
C
 ...
 1101 FORMAT(/2X,'Energy',7X,'CS0,e-',6X,'CS1,e-',6X,'CS2,e-',6X,
     ...
     3  '(cm**2)',/1X,84('-'))
      DO I=1,NDATA
        READ(IRD,*) EJT(I),XE0(I),XE1(I),XE2(I),XP0(I),XP1(I),XP2(I)
        IF(INFO.GE.2) WRITE(IWR,'(1P,7E12.5)')
     1    EJT(I),XE0(I),XE1(I),XE2(I),XP0(I),XP1(I),XP2(I)
        EJTL(I)=LOG(EJT(I))
      ENDDO
C
...
C
      DO I=1,NDATA
        FJL(I)=LOG(XE0(I))
      ENDDO
      CALL SPLINE(EJTL,FJL,A,B,C,D,0.0D0,0.0D0,NDATA)
      DO I=1,NEGP
        EC=DLEMP(I)
        CALL FINDI(EJTL,EC,NDATA,J)
...
C
      DO IEL=1,NELEM(M)
        ...
        DO I=1,NEGP
          READ(3,*,END=1) EJT(I),XE0P,XE1P,XE2P,XP0P,XP1P,XP2P
...
C
 ...
 2001 FORMAT(1X,'***  Electron and positron elastic cross sections',
     1  ',  NDATA =',I4)
      DO I=1,NPTAB
        WRITE(IWR,2002) EJT(I),XE0(I),XE1(I),XE2(I),XP0(I),XP1(I),
     1    XP2(I)
      ENDDO
...
C
      ...
      ILBA(4)=0
      ILBA(5)=ILB(5)
      CALL STORES(REV,X,Y,Z,US,VS,WS,WGHT,2,ILBA,0)
      CALL STORES(REV,X,Y,Z,-US,-VS,-WS,WGHT,2,ILBA,0)
C
      RETURN
      END
...
C  ****  Initial direction (isotropic).
        ...
        ILBA(5)=ILB(5)
        CALL STORES(ET(KS),X,Y,Z,US,VS,WS,WGHT,KPARS,ILBA,0)
      ENDIF
C
...
C
C                                       Francesc Salvat. 19 March, 2011.
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
        ...
        IF(ABS(HH).LT.TOL3*MAX(ABS(X1),ABS(X2))) THEN
C  ****  Subintervals are too narrow (possible roundoff errors).
          SUMR=SUMR+S12
          INEXT=I+1
          IERGA=1
          GO TO 2
...
C
    2 CONTINUE
      ...
      SUMGA=SUMGA+SUMR
      IF(IERGA.EQ.2) THEN
        IF(NOI.LT.10) THEN  ! Less than 10 narrow intervals.
          IERGA=0  ! The result is expected to be correct.
          RETURN
        ENDIF
      ENDIF
      WRITE(6,11)
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (NR=128)
      COMMON/CRNDG3/X(NR),A(NR),B(NR),F(NR),IA(NR),NPM1
      EXTERNAL RAND
...
C  ****  Sampling from the rational inverse cumulative distribution.
      IF(RR.GT.1.0D-12) THEN
        RNDG3=X(I)+((1.0D0+A(I)+B(I))*D*RR/(D*D+(A(I)*D+B(I)*RR)*RR))
     1      *(X(I+1)-X(I))
      ELSE
        RNDG3=X(I)+RAND(2.0D0)*(X(I+1)-X(I))
      ENDIF
C
      RETURN
      END
...
C  Initialisation of the RNDG3 sampling function.
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (NM=512)
      COMMON/CRITAA/XA(NM),AA(NM),BA(NM),FA(NM),IAA(NM),NPM1A
      PARAMETER (NR=128)
      COMMON/CRNDG3/X(NR),A(NR),B(NR),F(NR),IA(NR),NPM1
      EXTERNAL RNDG3F
C
      ...
      XMIN=-3.0D0
      XMAX=+3.0D0
      CALL RITA0(RNDG3F,XMIN,XMAX,N,NU,ERRM,0)
      IF(N.NE.NR) CALL PISTOP('RNDG30: initialisation error (1).')
      IF(ERRM.GT.1.0D-6)
     1   CALL PISTOP('RNDG30: initialisation error (2).')
C
      DO I=1,NR
        X(I)=XA(I)
        A(I)=AA(I)
...
C  ****  Secondary stack.
     ...
     1   VS(NMS),WS(NMS),WGHTS(NMS),SP1S(NMS),SP2S(NMS),SP3S(NMS),
...
C  ****  Delta ray.
      IF(ES.GT.EABS(1,M)) THEN
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(ES,X,Y,Z,US,VS,WS,WGHT,1,ILBA,0)
      ENDIF
...
C  ****  Bremsstrahlung photon.
      IF(DE.GT.EABS(2,M)) THEN
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(DE,X,Y,Z,US,VS,WS,WGHT,2,ILBA,0)
      ENDIF
C  ****  New energy.
      E=E-DE
...
C  ****  Electron.
      IF(EE.GT.EABS(1,M)) THEN
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(EE,X,Y,Z,US,VS,WS,WGHT,1,ILBA,0)
      ENDIF
C  ****  Positron.
      IF(EP.GT.EABS(3,M)) THEN
        DF=TWOPI*RAND(7.0D0)
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(EP,X,Y,Z,U,V,W,WGHT,3,ILBA,0)
...
C
 3600 ICOL=6
      ...
      IF(E1.GT.EABS(2,M)) THEN
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(E1,X,Y,Z,US,VS,WS,WGHT,2,ILBA,0)
      ENDIF
      IF(E2.GT.EABS(2,M)) THEN
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(E2,X,Y,Z,US,VS,WS,WGHT,2,ILBA,0)
      ENDIF
      DE=E+TREV
      E=0.0D0
      RETURN
...
C                       SUBROUTINE STORES
C  *********************************************************************
      SUBROUTINE STORES(EI,XI,YI,ZI,UI,VI,WI,WGHTI,KPARI,ILBI,IPOLI)
C
...
C  ****  Positron simulation tables.
     ...
     6  T1P(MAXMAT,NEGP),T2P(MAXMAT,NEGP)
C  ****  Total and transport cross sections.
      COMMON/CEEL01/EJT(NEGP),XE0(NEGP),XE1(NEGP),XE2(NEGP),XP0(NEGP),
     1  XP1(NEGP),XP2(NEGP),T1E0(NEGP),T2E0(NEGP),T1P0(NEGP),T2P0(NEGP),
     2  EJTL(NEGP),FL(NEGP),A(NEGP),B(NEGP),C(NEGP),D(NEGP)
C
C  ****  Elastic scattering simulation tables.
C
      PARAMETER (NE=96,NA=606)
...
C
 ...
 2001 FORMAT(/1X,'***  Electron elastic differential cross sections')
     ...
     1    IELEC,ETSIE,ECS(IE),ETCS1(IE),ETCS2(IE)
        IF(INFO.GT.3) WRITE(IWR,'(I3,1P,E10.3,5E12.5)')
     1    IELEC,ETS(IE),ECS(IE),ETCS1(IE),ETCS2(IE)
        ...
        IF(INFO.GT.3) WRITE(IWR,'(1P,10E12.5)') (EDCS(IE,K),K=1,NA)
C  ****  Consistency test.
        DO K=1,NA
          DCSI(K)=EDCS(IE,K)
        ENDDO
...
C
 ...
 2002 FORMAT(/1X,'***  Positron elastic differential cross sections')
     ...
     1    IELEC,ETS(IE),PCS(IE),PTCS1(IE),PTCS2(IE)
        ...
        IF(INFO.GT.3) WRITE(IWR,'(1P,10E12.5)') (PDCS(IE,K),K=1,NA)
C  ****  Consistency test.
        DO K=1,NA
          DCSI(K)=PDCS(IE,K)
        ENDDO
