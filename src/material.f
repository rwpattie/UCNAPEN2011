...
C
C* PDATCONF.P08: atomic ground-state configurations, ionisation energies
C     and central values of the one-electron shell Compton profiles for
C     the elements, from hydrogen to einsteinium (Z=1-99).
C
C* PDCOMPOS.P08: prepared composition data for 280 different materials
C     of radiological interest (adapted from Berger, NISTIR 4999, 1992).
C
C* PDEFLIST.P08: list of materials included in the PDCOMPOS.P08 file,
C     with their identification numbers.
C
C* PDRELAX.P11: data on atomic relaxation, extracted from the LLNL
C     Evaluated Atomic Data Library.
C
C* 99 files named PDEELZZ.P08 with ZZ=atomic number (001-099). These
...
C     elements.
C
C* 99 files named PDGPHZZ.P11, containing total atomic photoelectric
C     cross sections and partial cross sections for inner (K, L and M)
C     shells, generated from the EPDL97 data library of Cullen et al.
C
C* 99 files named PDESIZZ.P11 with cross sections for ionisation of
C     inner (K, L and M) shells by electron impact, generated from the
C     distorted-wave and plane-wave Born approximations.
C
C* 99 files named PDPSIZZ.P11 with cross sections for ionisation of
C     inner (K, L and M) shells by positron impact, generated from the
C     distorted-wave and plane-wave Born approximations.
C
...
C  read from the 'pdcompos.p08' file, which contains information for 280
